...
def extraccion_1(directorio= '../netCDF/*.nc', directorio_raiz = '../netCDF/', latt =  6.646697 , lonn = -75.460673, V = 'pr', C = 'preipitacion',F_C = 86400):
    ...
    Nombres_originales = os.listdir(directorio_raiz)
    ...
    for nombre_original, año in zip(Nombres_originales, all_year):
        ...
        ruta_original = os.path.join(directorio_raiz, nombre_original)
        ruta_nueva = os.path.join(directorio_raiz, nombre_nuevo)
    ...
    df = pd.DataFrame(0.0 , columns= [C], index = dates )
    ...
    for i in all_year:
        ...
        for x in np.arange(0, len(dates)):
            df.loc[dates[x]][C] = prec[x, min_lat , min_lon]*F_C
            
        print("Se ejecuto de forma exitosa")

    df.to_csv('../CSV_OUT/generales/santarosa_P2.csv')
    

        


def multiple_extraccion(directorio= '../netCDF/*.nc', directorio_raiz = '../netCDF', ubixaciones = '../XLSM/UBICACIONES.csv', V = 'pr', C = 'preipitacion'):
    # *! identidicador de fechas automatico
    all_year = []# **con esto pordemo ver si glob esta leyendo y uniendo dentro de data todos los netCDF
    ...
    for index, row in locations.iterrows():
        ...
        for i in all_year:
            ...
            for x in np.arange(0, len(dates)):
                df.loc[dates[x]][C] = prec[x, min_lat , min_lon]
...
def multiple_extraccion2(directorio= '../netCDF/*.nc', directorio_raiz = '../netCDF', ubixaciones = '../XLSM/UBICACIONES.csv', V = 'pr', C = 'preipitacion'):
